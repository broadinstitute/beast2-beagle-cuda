...
function print_usage(){
  ...
  echo ""
  echo "  Note: The version of BEAST2 used should match the version of BEAUTi"
  echo "        used to generate the input xml file."
  echo ""
  echo "        Docker images have been built for several versions of BEAST2."
  ...
  echo ""
  echo "        Extra arguments for BEAST2 may be passed via the BEAST_EXTRA_ARGS environment variable."
  echo "        For example:"
...
# when we have auto-partition code again, [ $# -lt 3 ]
if [ $# -eq 0 ] || [ $# -lt 4 ]; then
    print_usage
    exit 1
fi
...
if [[ -z "${BEAST_VERSION}" ]]; then
  DOCKER_IMAGE_TAG=":2.5.0" # temporary until BADTRIP and others can run with 2.6.x; then ":latest"
else
...
if [ -z "$4" ]; then
  # ===== override auto-partition code with value from user until we know how to parse beast2 xml files for partition count
  # number_of_partitions=$(gsutil cat "$1" | grep "<distribution idref>" | wc -l | awk '{ printf "%d\n", $0 }')
  ...
  # ===== end override auto-partition code with value from user until we know how to parse beast2 xml files for partition count
  BEAGLE_ORDER="$4"
else
  BEAGLE_ORDER="$4"
fi
...
dsub \
  ...
  --script "${SCRIPTPATH}/run_beast2.sh"  \
  --boot-disk-size 15 \
  ${ACCELERATOR_SPEC}
  #--wait
